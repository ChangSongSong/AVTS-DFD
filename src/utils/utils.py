...
def Contrastive(vid_out, aud_out, target, hyper_param):
    ...
    dists_square = torch.nan_to_num(torch.sum(torch.square(vid_out - aud_out), -1))
    dists = torch.sqrt(dists_square)

    real_loss = (torch.ones_like(target) - target).squeeze(-1) * dists_square
    fake_loss = target.squeeze(-1) * F.relu((torch.ones_like(dists)*hyper_param - dists))**2
...
def Triplet(real_vid_out, fake_vid_out, aud_out, hyper_param):
    ...
    for i in range(batch_size):
        real_dist = torch.dist(real_vid_out[i, :].view(-1), aud_out[i, :].view(-1), 2)
        fake_dist = torch.dist(fake_vid_out[i, :].view(-1), aud_out[i, :].view(-1), 2)
...
def compute_video_level_acc(video_to_logits, video_to_labels):
    ...
    sig = nn.Sigmoid()
    acc = (torch.sum((sig(output_batch) > 0.5) == output_labels) / output_batch.shape[0]).item()
...
def compute_clip_level_acc(video_to_logits, video_to_labels):
    output_batch = torch.cat([torch.stack(video_to_logits[video_id]) for video_id in video_to_logits])
    output_labels = torch.cat([torch.stack([video_to_labels[video_id]]*len(video_to_logits[video_id])) for video_id in video_to_logits])
...
def compute_clip_level_auc(video_to_logits, video_to_labels):
    output_batch = torch.cat([torch.stack(video_to_logits[video_id]) for video_id in video_to_logits]).numpy()
    output_labels = torch.cat([torch.stack([video_to_labels[video_id]]*len(video_to_logits[video_id])) for video_id in video_to_logits]).numpy()
