...
class Tester():
    ...
    def __init__(self, config):
        # Parameters
        self.config = config
        self.device = torch.device(f"cuda:{config['test']['gpu_ids'][0]}"
        ...
        self.model = FakeNet(
            ...
            frames_per_clip=int(config['dataset']['fps'] *
        ...
        # resume model
        checkpoint = torch.load(self.config['test']['resume'],
                                map_location=self.device)

        if 'module' in list(checkpoint['model'].keys())[0]:
            self.model = torch.nn.DataParallel(
                self.model,
                device_ids=config['test']['gpu_ids']).to(self.device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for it, data in enumerate(tqdm(self.test_loader)):
                ...
                x = list(map(lambda k: k.to(self.device, dtype=torch.float),
                             x))
                ...
                if self.use_predict == 'fc':
                    ...
                    logits = torch.sqrt(
                        torch.nan_to_num(
                            torch.sum(torch.square(v_feats - a_feats), 1)))
                ...
                for i in range(len(vpath)):
                    ...
                    video_to_logits[video_id].append(
                        logits[i].view(-1).detach().cpu())
                    video_to_probs[video_id].append(
                        sig(logits[i]).view(-1).detach().cpu())
        ...
        precision_video, recall_video, f1score_video = compute_video_level_precision_recall_F1score(
            video_to_logits, video_to_labels)
