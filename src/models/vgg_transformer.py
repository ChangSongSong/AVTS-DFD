...
class VGGTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.change_to_dim = nn.Sequential(
            nn.ReLU(),
            nn.Linear(128, last_dim),
        )

        self.pos_embedding = nn.Parameter(
            torch.randn(1, frames_per_clip, last_dim))
        self.dropout = nn.Dropout(emb_dropout)
        self.transformer = Transformer(last_dim, depth, heads, dim_head,
                                       mlp_dim, dropout)
        self.to_latent = nn.Identity()

        self.mlp_head = nn.Sequential(nn.ReLU(), nn.Linear(last_dim, last_dim),
                                      nn.ReLU(), nn.Linear(last_dim, last_dim),
                                      nn.ReLU(), nn.Linear(last_dim, last_dim))
