...
class AVMNet(nn.Module):
    ...
    def __init__(self,
        ...
        # Teacher
        if self.use_teacher:
            self.v_teacher = self._select_backbone('v_encoder',
                                                   self.backbone[0])
            self.a_teacher = self._select_backbone('a_encoder',
                                                   self.backbone[1])
        ...
        if self.setting == 'AVM':
            self.avts_v_fc = nn.Sequential(
                nn.ReLU(), nn.Linear(self.last_dim, self.last_dim))
    ...
    def forward(self, vid, aud, alpha=0, y=None, out_feat=False):
        ...
        if self.use_teacher:
            v_feats_teacher, a_feats_teacher = self.forward_feat(
                vid, aud, is_teacher=True)
            if 'L2' in self.use_losses:
            ...
            if 'NTXent' in self.use_losses:
                if self.setting == 'AVTS':
                    ...
                    labels = torch.FloatTensor(
                        torch.arange(clips).repeat(2).float()).to(
                            feats.get_device())
                    metric_loss = 0
                    # only compute loss in the same video
                    for i in range(batch_size):
                        metric_loss += self.NTXentLoss(feats[i],
                                                       labels) / batch_size
