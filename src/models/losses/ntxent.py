...
class NTXentLoss(GenericPairLoss):
    ...
    def __init__(self, temperature=0.07, gamma=0, **kwargs):
        ...
        self.gamma = gamma
        self.add_to_recordable_attributes(list_of_names=["temperature"],
                                          is_stat=False)
    ...
    def _compute_loss(self, pos_pairs, neg_pairs, indices_tuple):
        ...
        if len(a1) > 0 and len(a2) > 0:
            ...
            n_per_p = c_f.to_dtype(a2.unsqueeze(0) == a1.unsqueeze(1),
                                   dtype=dtype)
            neg_pairs = neg_pairs * n_per_p
            neg_pairs[n_per_p == 0] = c_f.neg_inf(dtype)

            max_val = torch.max(pos_pairs,
                                torch.max(neg_pairs, dim=1,
                                          keepdim=True)[0]).detach()
            numerator = torch.exp(pos_pairs - max_val).squeeze(1)
            denominator = torch.sum(torch.exp(neg_pairs - max_val),
                                    dim=1) + numerator
