...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_embedding = nn.Parameter(
            torch.randn(1, frames_per_clip + 1, dim))
        ...
        if self.modality_embedding:
            self.v_embedding = nn.Parameter(
                torch.randn(1, frames_per_clip + 1, dim // 2))
    ...
    def forward(self, x, predict_fake_label=False, alpha=0):
        ...
        if self.modality_embedding:
            x += torch.cat((self.v_embedding, self.a_embedding),
                           dim=-1)[:, :(n + 1)]
            x = self.dropout2(x)
        ...
        if predict_fake_label:
            ...
            return self.mlp_head(x), self.fake_type_mlp_head(
                reverse_x), feature
        else:
